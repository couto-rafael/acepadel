...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose }) => {
  ...
  const validateEmail = (email: string): { isValid: boolean; error?: string } => {
    ...
    if (cleanEmail.length < 5) {
      return { isValid: false, error: 'Email muito curto. Mínimo de 5 caracteres.' };
    }
    
    // Check maximum length
    if (cleanEmail.length > 254) {
      return { isValid: false, error: 'Email muito longo. Máximo de 254 caracteres.' };
    }
    ...
    if (!emailRegex.test(cleanEmail)) {
      return { isValid: false, error: 'Formato de email inválido. Use o formato: usuario@exemplo.com' };
    }
    
    // Split email into local and domain parts
    const parts = cleanEmail.split('@');
    if (parts.length !== 2) {
      return { isValid: false, error: 'Email deve conter exatamente um símbolo @.' };
    }
    ...
    if (localPart.length > 64) {
      return { isValid: false, error: 'Parte antes do @ muito longa (máximo 64 caracteres).' };
    }
    ...
    if (domain.length > 253) {
      return { isValid: false, error: 'Domínio muito longo (máximo 253 caracteres).' };
    }
    ...
    for (const part of domainParts) {
      if (part.length === 0) {
        return { isValid: false, error: 'Domínio não pode ter partes vazias.' };
      }
      
      if (part.startsWith('-') || part.endsWith('-')) {
        return { isValid: false, error: 'Partes do domínio não podem começar ou terminar com hífen.' };
      }
      
      // Check if part contains only valid characters
      if (!/^[a-zA-Z0-9-]+$/.test(part)) {
        return { isValid: false, error: 'Domínio contém caracteres inválidos.' };
      }
    }
    ...
    if (tld.length < 2) {
      return { isValid: false, error: 'Extensão do domínio deve ter pelo menos 2 caracteres.' };
    }
    ...
    // Check for invalid characters that might cause Supabase issues
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        // The App.tsx will handle the redirection based on profile data
        ...
        // After successful signup, redirect to appropriate dashboard
        if (userType === 'club') {
